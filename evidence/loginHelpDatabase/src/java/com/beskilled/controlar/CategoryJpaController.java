    ...
    public void create(Category category) throws RollbackFailureException, Exception {
        ...
        try {
            ...
            for (Product productCollectionProduct : category.getProductCollection()) {
                Category oldCatIdOfProductCollectionProduct = productCollectionProduct.getCatId();
                productCollectionProduct.setCatId(category);
                productCollectionProduct = em.merge(productCollectionProduct);
                if (oldCatIdOfProductCollectionProduct != null) {
                    oldCatIdOfProductCollectionProduct.getProductCollection().remove(productCollectionProduct);
                    oldCatIdOfProductCollectionProduct = em.merge(oldCatIdOfProductCollectionProduct);
                }
            }
            utx.commit();
        } catch (Exception ex) {
        ...
        try {
            ...
            for (Product productCollectionOldProduct : productCollectionOld) {
                if (!productCollectionNew.contains(productCollectionOldProduct)) {
                    productCollectionOldProduct.setCatId(null);
            ...
            for (Product productCollectionNewProduct : productCollectionNew) {
                if (!productCollectionOld.contains(productCollectionNewProduct)) {
                    Category oldCatIdOfProductCollectionNewProduct = productCollectionNewProduct.getCatId();
                    productCollectionNewProduct.setCatId(category);
                    productCollectionNewProduct = em.merge(productCollectionNewProduct);
                    if (oldCatIdOfProductCollectionNewProduct != null && !oldCatIdOfProductCollectionNewProduct.equals(category)) {
                        oldCatIdOfProductCollectionNewProduct.getProductCollection().remove(productCollectionNewProduct);
                        oldCatIdOfProductCollectionNewProduct = em.merge(oldCatIdOfProductCollectionNewProduct);
                    }
                }
            }
            utx.commit();
    ...
    public void destroy(Integer id) throws NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            for (Product productCollectionProduct : productCollection) {
                productCollectionProduct.setCatId(null);
                productCollectionProduct = em.merge(productCollectionProduct);
            }
            em.remove(category);
            utx.commit();
